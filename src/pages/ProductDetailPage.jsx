...
export default function ProductDetailPage() {
  ...
  if (loading) {
    return (
      <div style={{ backgroundColor: '#d5e0e8' }} className="min-h-screen pt-10">
        <div className="max-w-6xl mx-auto px-6 py-14 grid grid-cols-1 md:grid-cols-2 gap-12">
          <div className="rounded-3xl bg-neutral-300 animate-pulse aspect-[3/4]" />
          <div className="flex flex-col gap-4 pt-4">
            <div className="h-4 w-24 bg-neutral-300 animate-pulse rounded-full" />
            <div className="h-10 w-3/4 bg-neutral-300 animate-pulse rounded-xl" />
            <div className="h-6 w-1/4 bg-neutral-300 animate-pulse rounded-full" />
            <div className="h-32 bg-neutral-300 animate-pulse rounded-xl mt-4" />
          </div>
        </div>
      </div>
    )
  ...
  if (error) {
    return (
      <div style={{ backgroundColor: '#d5e0e8' }} className="min-h-screen flex items-center justify-center">
    ...
    <div style={{ backgroundColor: '#d5e0e8' }} className="min-h-screen pt-10">
      <div className="max-w-6xl mx-auto px-6 py-10">
        ...
        <nav className="flex items-center gap-2 text-[11px] text-gray-500 mb-8 tracking-wider">
          ...
          <span>/</span>
          <span className="text-gray-800 font-semibold uppercase truncate max-w-[200px]">{product.title}</span>
        </nav>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          ...
          <div className="flex gap-3">
            ...
            {/* Main image */}
            <div className="flex-1 rounded-3xl overflow-hidden bg-neutral-200 aspect-[3/4] relative">
              <img
              ...
              />
              {/* Category badge */}
              <span className="absolute top-4 left-4 bg-white/80 backdrop-blur-sm text-[10px] font-semibold tracking-wider px-3 py-1 rounded-full text-gray-800">
          ...
          <div className="flex flex-col">
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-full text-xs font-bold border-2 transition-all ${
                      selectedSize === size
                        ...
                        : 'bg-white/60 text-gray-700 border-gray-200 hover:border-gray-900'
                    }`}
                  >
                    {size}
                  </button>
            ...
            <div className="flex items-center gap-3 mb-4">
              {/* Quantity */}
              <div className="flex items-center bg-white/60 rounded-full border border-gray-200">
                <button
                  onClick={() => setQuantity((q) => Math.max(1, q - 1))}
                  className="px-3 py-2 text-gray-700 hover:text-gray-900 font-bold"
                >
                  −
                </button>
            ...
            {/* Wishlist */}
            <button className="w-full py-3 rounded-full text-xs font-bold tracking-widest border-2 border-gray-300 text-gray-700 hover:border-gray-900 hover:text-gray-900 transition-all mb-8">
              ADD TO WISHLIST
            </button>

            {/* Details accordion-style info */}
            <div className="border-t border-gray-300 pt-5 flex flex-col gap-4 text-[11px] text-gray-500">
        ...
        <div className="mt-14 pb-4">
          <button
            onClick={() => navigate(-1)}
            className="flex items-center gap-2 text-xs font-bold tracking-widest text-gray-600 hover:text-gray-900 transition-colors"
          >
